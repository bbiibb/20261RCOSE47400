...
class Spectrogram(nn.Module):
    def __init__(self, sr=32000, n_fft=2048, n_mels=256, hop_length=512, f_min=20, f_max=16000, channels=1, norm="slaney", mel_scale="htk", target_size=(256, 256), top_db=80.0, delta_win=5, spec_aug=None):
        super().__init__()
        ...
        self.freq_mask = torchaudio.transforms.FrequencyMasking(self.spec_aug.get("freq_mask_param", 0))
        self.time_mask = torchaudio.transforms.TimeMasking(self.spec_aug.get("time_mask_param", 0))
    ...
    def forward(self, x):
        ...
        if self.training and self.spec_aug_enabled and torch.rand(1).item() < self.spec_aug.get("p", 0.3):
            for _ in range(self.spec_aug.get("num_masks", 1)):
...
class Trainer:
    ...
    def __init__(self, config={}, fold=0, epochs=16):
        ...
        self.mel = Spectrogram(**self.config["mel"], spec_aug=self.config.get("spec_aug")).to(self.device)
    ...
    def train(self, epochs=16, checkpoint_freq='once'):
        ...
        for epoch in Epochs:
            ...
            if not self.config["train_only"]:
                print(
                    f"\033[1m Validation \t|\t loss={np.round(val_loss, 3)}  -  "
                    + "  -  ".join([
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "spec_aug": {"enable": False, "p": 0.3, "freq_mask_param": 12, "time_mask_param": 24, "num_masks": 1}, 
